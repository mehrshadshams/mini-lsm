...
impl SstConcatIterator {
    pub fn create_and_seek_to_first(sstables: Vec<Arc<SsTable>>) -> Result<Self> {
        ...
        let iterator =
            SsTableIterator::create_and_seek_to_first(sstables.first().unwrap().clone())?;
        let mut it = Self {
    ...
    pub fn create_and_seek_to_key(sstables: Vec<Arc<SsTable>>, key: KeySlice) -> Result<Self> {
        ...
        let iterator = SsTableIterator::create_and_seek_to_key(sstables[idx].clone(), key)?;
        let mut it = Self {
            current: Some(iterator),
            next_sst_idx: idx + 1,
            sstables,
